...
App::import('Controller', 'HomeworkProgressions');
class ProgressionController extends AppController
{
    ...
    public function index()
    {
        ...
        $HomeworkProgressions = new HomeworkProgressionsController;
        $HomeworkProgressions->constructClasses();
        ...
        $totalExamGiven=$this->Progression->find('count',array('conditions'=>array('Progression.student_id'=>$this->studentId)));
        $totalHWGiven=$HomeworkProgressions->HomeworkProgression->find('count',array('conditions'=>array('HomeworkProgression.student_id'=>$this->studentId)));
        $failedExam=$this->Progression->find('count',array('conditions'=>array('Progression.student_id'=>$this->studentId,'Progression.result'=>'Fail')));
        $userTotalAbsent=$this->Progression->userTotalAbsent($this->studentId);
        if($userTotalAbsent<0)
        $userTotalAbsent=0;
        //best score exam
        $bestScoreArr=$this->Progression->userBestExam($this->studentId);
        $bestScore="";
        $bestScoreDate="";
        ...
        // best score homework
        $bestScoreHWArr=$HomeworkProgressions->HomeworkProgression->userBestHomework($this->studentId);
        $bestScoreHW="";
        $bestScoreHWDate="";
        if(isset($bestScoreHWArr['Homework']['name']))
        {
            $bestScoreHW=$bestScoreHWArr['Homework']['name'];
            $bestScoreHWDate=CakeTime::format($this->sysDay.$this->dateSep.$this->sysMonth.$this->dateSep.$this->sysYear.$this->dateGap.$this->sysHour.$this->timeSep.$this->sysMin.$this->dateGap.$this->sysMer,$bestScoreHWArr['HomeworkResult']['start_time']);
        }
        // worst score homework
        $worstScoreHWArr=$HomeworkProgressions->HomeworkProgression->userWorstHomework($this->studentId);
        $worstScoreHW="";
        $worstScoreHWDate="";
        if(isset($worstScoreHWArr['Homework']['name']))
        {
            $worstScoreHW=$worstScoreHWArr['Homework']['name'];
            $worstScoreHWDate=CakeTime::format($this->sysDay.$this->dateSep.$this->sysMonth.$this->dateSep.$this->sysYear.$this->dateGap.$this->sysHour.$this->timeSep.$this->sysMin.$this->dateGap.$this->sysMer,$worstScoreHWArr['HomeworkResult']['start_time']);
        }
        // Missed Homework
        $missedHW = $HomeworkProgressions->HomeworkProgression->userMissedHomework($this->studentId);
        ...
        $this->set('bestScoreHW',$bestScoreHW);
        $this->set('bestScoreHWDate',$bestScoreHWDate);
        $this->set('worstScoreHW',$worstScoreHW);
        $this->set('worstScoreHWDate',$worstScoreHWDate);
        $this->set('missedHW',$missedHW);
