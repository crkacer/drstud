...
class InstallersController extends AppController {
	...
	if(!$file->writable())
	$this->set('dbfile','<span class="label label-danger">'.__('Unwriteable').'</span>');
	else
	$this->set('dbfile','<span class="label label-success">'.__('Writeable').'</span>');
	$file->close();
	$file = new File(APP.'/Config/core.php');
	if(!$file->writable())
	$this->set('corefile','<span class="label label-danger">'.__('Unwriteable').'</span>');
	else
	$this->set('corefile','<span class="label label-success">'.__('Writeable').'</span>');
	$file->close();
	$file = new File(TMP.'temp.txt',true,0777);
	if(!$file->writable())
	$this->set('tmpfile','<span class="label label-danger">'.__('Unwriteable').'</span>');
	else
	$this->set('tmpfile','<span class="label label-success">'.__('Writeable').'</span>');
	$file->delete();
	$file->close();	
    }
