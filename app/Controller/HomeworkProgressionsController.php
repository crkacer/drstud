...
class HomeworkProgressionController extends AppController
{
    ...
    public function index()
    {
        ...
        $totalHWGiven=$this->Progression->find('count',array('conditions'=>array('HomeworkProgression.student_id'=>$this->studentId)));
        $failedHomework=$this->Progression->find('count',array('conditions'=>array('Progression.student_id'=>$this->studentId,'Progression.result'=>'Fail')));
        $userTotalAbsent=$this->HomeworkProgression->userTotalAbsent($this->studentId);
        if($userTotalAbsent<0)
        $userTotalAbsent=0;
        // Best score homework
        $bestScoreArr=$this->HomeworkProgression->userBestHomework($this->studentId);
        $bestScore="";
        $bestScoreDate="";
        if(isset($bestScoreArr['Homework']['name']))
        {
            $bestScore=$bestScoreArr['Homework']['name'];
            $bestScoreDate=CakeTime::format($this->sysDay.$this->dateSep.$this->sysMonth.$this->dateSep.$this->sysYear.$this->dateGap.$this->sysHour.$this->timeSep.$this->sysMin.$this->dateGap.$this->sysMer,$bestScoreArr['HomeworkResult']['start_time']);
        }
        // Worst score homework
        $worstScoreArr=$this->HomeworkProgression->userWorstHomework($this->studentId);
        $worstScore="";
        $worstScoreDate="";
        if(isset($worstScoreArr['Homework']['name']))
        {
            $worstScore=$worstScoreArr['Homework']['name'];
            $worstScoreDate=CakeTime::format($this->sysDay.$this->dateSep.$this->sysMonth.$this->dateSep.$this->sysYear.$this->dateGap.$this->sysHour.$this->timeSep.$this->sysMin.$this->dateGap.$this->sysMer,$worstScoreArr['HomeworkResult']['start_time']);
        }

        $this->set('limit',$this->limit);
        $this->set('totalHWGiven',$totalHWGiven);
        $this->set('failedHomework',$failedHomework);
        $this->set('userTotalAbsent',$userTotalAbsent);
        $this->set('bestScore',$bestScore);
        $this->set('bestScoreDate',$bestScoreDate);
        $this->set('worstScore',$worstScore);
        $this->set('worstScoreDate',$worstScoreDate);
        ...
        for($i=1;$i<=12;$i++)
        {
          if($i>$currentMonth)
          break;
          $homeworkData=$this->HomeworkProgression->performanceCount($this->studentId,$i);
          $performanceChartData[]=(float) $homeworkData;
        }
        ...
        $this->loadModel('HomeworkResult');
        $homeworkResultArr=$this->HomeworkResult->find('all',array('fields'=>array('Homework.name','HomeworkResult.percent'),
                                                           'joins'=>array(array('table'=>'homeworks','alias'=>'Homework','type'=>'INNER','conditions'=>array('HomeworkResult.homework_id=Homework.id'))),
                                                           'conditions'=>array('HomeworkResult.student_id'=>$this->studentId),
                                                           'order'=>array('HomeworkResult.id'=>'desc'),
                                                           'limit'=>10));
        $this->HomeworkResult->virtualFields=array('total_percent'=>'SUM(HomeworkResult.percent)');
        $totalPercentArr=$this->HomeworkResult->find('first',array('fields'=>array('total_percent'),'conditions'=>array('HomeworkResult.student_id'=>$this->studentId)));
        $this->HomeworkResult->virtualFields=array();
        $totalHomeworkAttempt=$this->HomeworkResult->find('count',array('conditions'=>array('HomeworkResult.student_id'=>$this->studentId)));
        $totalPercent=$totalPercentArr['HomeworkResult']['total_percent'];
        if($totalhomeworkAttempt>0)
        $averagePercent=round($totalPercent/$totalhomeworkAttempt,2);
        else
        $averagePercent=0;
        $performanceChartData=array();$xAxisCategories=array();
        foreach($homeworkResultArr as $post)
        {
           $xAxisCategories[]=array($post['Homework']['name']);
           $performanceChartData[]=array((float) $post['HomeworkResult']['percent']);
        }
        ...
        $rank=0;
        $rankPost=$this->HomeworkResult->query("SELECT `percent`,`student_id`, FIND_IN_SET(`percent`,(SELECT GROUP_CONCAT(`percent` ORDER BY `percent` DESC) FROM `homework_results`)) AS `rank` FROM `homework_results`  WHERE `student_id`=$this->studentId HAVING `rank` IS NOT NULL ORDER BY `percent` DESC LIMIT 1");
        if($rankPost)
