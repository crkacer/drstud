 ...
 */
	public function build($id = null, $options = array(), &$data = null, &$isActive = false) {
		...
		$class = (isset($id) && ($id != 'children')) ? ' id="'.$ulId.'" class="'.$this->settings['menuClass'].'"' : '';
		$class.= (isset($id) && ($id == 'children')) ? ' class="'.$this->settings['childrenDropdown'].'"' : '';
	...
	protected function _buildItem(&$item, $pos = -1, &$isActive = false) {
		...
		if (isset($item['url'])) {
			if ($item['partialMatch']) {
				$check = (strpos(Router::normalize(strtolower($this->here)), Router::normalize(strtolower($item['url']['controller']))));
				if($check==false)
				$check = (strpos(Router::normalize($this->here), Router::normalize($item['selName'])));
			} else {
		...
		if (is_null($item['url']['controller'])) {
