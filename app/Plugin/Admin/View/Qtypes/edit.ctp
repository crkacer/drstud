<div <?php if(!$isError){?>class="container"<?php }?>>    
    <div class="panel panel-custom mrg">
        <div class="panel-heading"><?php echo __('Edit');?> <?php echo __('Question Type');?><?php if(!$isError){?><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><?php }?></div>
        <div class="panel-body"><?php echo $this->Session->flash();?>
	    <?php echo $this->Form->create('Qtype',array('class'=>'form-horizontal'));?>
	    <?php foreach ($Qtype as $k=>$post): $id=$post['Qtype']['id'];$form_no=$k+1;?>
		...
		<div class="panel-body">
			...
			<div class="col-sm-9">
			   <?php echo $this->Form->input("$k.Qtype.question_type",array('label' => false,'class'=>'form-control','placeholder'=> __('Name'),'div'=>false));?>
			</div>
		    </div>
		    <div class="form-group text-left">
			<div class="col-sm-offset-3 col-sm-6">
			    <?php echo $this->Form->input("$k.Qtype.id", array('type' => 'hidden'));?>
			</div>
		    </div>
		</div>
	    </div>
