...
<div class="page-title"> <div class="title-env"> <h1 class="title"><?php echo __('Lessons');?></h1></div></div>
<div class="panel"><?php echo $this->Session->flash();?>
    <div class="panel-heading">
        <div class="btn-group">
            <?php $url=$this->Html->url(array('controller'=>'Lessons')); if($userPermissionArr['save_right']){ echo $this->Html->link('<span class="fa fa-plus-circle"></span>&nbsp;'.__('Add New Lesson'),array('controller'=>'Lessons','action'=>'add'),array('escape'=>false,'class'=>'btn btn-success'));}?>
...
<div class="panel-body">
			    ...
			    <th><?php echo $this->Paginator->sort('ordering', __('Sequence Order'), array('direction' => 'asc'));?></th>
                        ...
                        <?php foreach ($Lesson as $post):
                        $id=$post['Lesson']['id'];?>
                        <tr>
                            <td><?php echo $this->Form->checkbox(false,array('value' => $post['Lesson']['id'],'name'=>'data[Lesson][id][]','id'=>"DeleteCheckbox$id",'class'=>'chkselect'));?></td>
                            <td><?php echo $serial_no++; ?></td>
                            <td><?php echo h($post['Subject']['subject_name']); ?></td>
                            <td><?php echo h($post['Lesson']['name']); ?></td>
			    <td><?php echo $post['Lesson']['ordering']; ?></td>
                            <td><span class="label label-<?php if($post['Lesson']['status']=="Active")echo"success";else echo"danger";?>"><?php echo __($post['Lesson']['status']); ?></span></td>
