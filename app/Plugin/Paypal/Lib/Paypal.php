...
/***
 * SetExpressCheckoutRecurringPayments
 **/
public function setExpressCheckoutRecurringPayment($order) {
        try {
            // Build the NVPs
            $nvps = $this->buildExpressCheckoutNvp($order);
            ...
            $keysToAdd = array(
                ...
                'L_BILLINGAGREEMENTDESCRIPTION0' => $order['description'],
            );
 ...
 * Recurring Payments
 **/
    public function doExpressCheckoutRecurringPayment($order, $token ,$payerId, $billingFrequency,$profileStartDate,$description,$email) {
        try {
            // Build the NVPs
            $nvps = $this->buildExpressCheckoutNvp($order);
            ...
            // Handle the resposne
            if (isset($parsed['PROFILESTATUS']) && $parsed['ACK'] == "Success")  {
                return $parsed;
            }
            else if ($parsed['ACK'] == "Failure" && isset($parsed['L_LONGMESSAGE0']))  {
                if (in_array($parsed['L_ERRORCODE0'], $this->redirectErrors) && isset($parsed['TOKEN'])) {
